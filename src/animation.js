...
Animation.prototype.display = function(time)
    ...
    this.graphicsState.lights.push( new Light( vec4( 0, 100, 0, 1 ), Color( 1, 1, 1, 1 ), 100000 ) );
		...
		**********************************/                                     // From this point on down it's just some examples for you -- feel free to comment it all out.
		...
		var bee_transform = this.draw_bee(mult(tree_transform, translation(0, 3, 0)));
}	
...
// Draw bee. Return original matrix
Animation.prototype.draw_bee = function (model_transform) {
	var SCALE = 0.25;
    
	var VERTICAL_INITIAL = 2;
	var VERTICAL_MAGNITUDE = 2;
    var VERTICAL_PERIOD = 1000; 
	
    var DISTANCE_FROM_TREE = 10;
	var CIRCLE_PERIOD = 30;
    ...
    // Fly in circle
	var fly_transform = mult(model_transform, rotation(this.graphicsState.animation_time / -CIRCLE_PERIOD, 0, 1, 0));
    var bee_tranform = mult(fly_transform, translation(DISTANCE_FROM_TREE, VERTICAL_INITIAL, 0));

    // Move up and down
    bee_tranform = mult(bee_tranform, translation(0, Math.sin(this.graphicsState.animation_time / VERTICAL_PERIOD * 4) * VERTICAL_MAGNITUDE, 0));
